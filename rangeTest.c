...
static uint8_t txBuffer[RAIL_TX_BUFFER_SIZE] = { 0x01, 0x10, 0x02, 0x20, 0x03,
		0x30, 0x04, 0x40, 0x05, 0x50, };
...
RAIL_FrameType_t rangeTestFrameConf = { 0u,           //uint8_t offset
		0u,		  	    //uint8_t mask
 ...
 *****************************************************************************/
int RepeatCallbackRegister(void (*pFunction)(void*), void* pParameter,
...
void RAILCb_RxPacketReceived(void *rxPacketHandle) {
	...
	rangeTestPacket_t * pRxPacket =
			(rangeTestPacket_t *) (rxPacketInfo->dataPtr);
	//LED0_TOGGLE;
	...
	if (rangeTest.isRunning) {
		...
		rangeTest.PER = (pktsCnt) // Avoid zero division
				? (((float) (pktsCnt - pktsRcvd) * 100.0f) / pktsCnt) // Calculate PER
				:
...
uint32_t rangeTestCountBits(uint32_t u) {
	uint32_t uCount = u - ((u >> 1u) & 033333333333)
			- ((u >> 2u) & 011111111111);
...
void rangeTestMAClearAll() {
	rangeTest.maHistory[0u] = rangeTest.maHistory[1u] =
			rangeTest.maHistory[2u] = rangeTest.maHistory[3u] = 0u;
}
...
int main(void) {
	...
	uint32_t lastLitTick = 0u;

	UARTDRV_Init_t uartInit = USART_INIT;
	RAIL_Init_t railInitParams = { RAIL_RX_BUFFER_SIZE,
			rangeTestConf.radioXTALFreq };
	...
	GPIO_PinModeSet(5, 3, gpioModePushPull, 0);
	GPIO_DriveStrengthSet(5, gpioDriveStrengthWeakAlternateWeak);
	while (1u) {
		if ((halCommonGetInt32uMillisecondTick() - lastTick) >= 5u) {
			// 5ms loop
			if(!GPIO_PinInGet(gpioPortC, 9)) {
				 if(!GPIO_PinOutGet(5, 3))  {
					  GPIO_PinOutSet(5, 3);

				  }
				  else {
					  GPIO_PinOutClear(5, 3);
				  }
...
//			if ((halCommonGetInt32uMillisecondTick() - lastLitTick) >= 500u) {
//				  if(GPIO_PinOutGet(5, 3) == 0)  {
//					  GPIO_PinOutSet(5, 3);
//					  GPIO_PinModeSet(5, 3, gpioModePushPull, 1);
//					  GPIO_DriveStrengthSet(5, gpioDriveStrengthWeakAlternateWeak);
//				  }
//				  else {
//					  GPIO_PinOutClear(5, 3);
//				  }
//				  lastLitTick = halCommonGetInt32uMillisecondTick();
//			}
			lastTick = halCommonGetInt32uMillisecondTick();
		}
...
//			memcpy((void*) &rangeTestBuffered, (void*) &rangeTest,
//					sizeof(rangeTest));
...
//								"IdR:%u"//8
//								"\n",//1+1
//						rangeTestBuffered.pktsRcvd, rangeTestBuffered.pktsCRC,
//						rangeTestBuffered.pktsCnt, rangeTestBuffered.payload,
//						rangeTestBuffered.maSize, rangeTestBuffered.PER,
//						rangeTestBuffered.MA,
//						(int8_t) rangeTestBuffered.rssiLatch,
//						rangeTestBuffered.srcID, rangeTestBuffered.destID);
//			}
//
//			if (RADIO_MODE_TX == rangeTest.radioMode) {
//				sprintf(buff, "Sent, Actual:%u, Max:%u, IdS:%u, IdR:%u\n",
//						rangeTestBuffered.pktsSent, rangeTestBuffered.pktsReq,
//						rangeTestBuffered.srcID, rangeTestBuffered.destID);
//			}
